 ...
 * Last modified : Monday 26 October 2020 07:47:26 PM IST
 * Description   : 
 * ***********************************************************************/
...
#define IRQ_NO 12     /* ps2 mouse interrupt */
 ...
 * tasklet_name, tasklet_function, data
 */
DECLARE_TASKLET(tasklet, tasklet_func, 1);

static void tasklet_func(unsigned long data)
{
	pr_info("Executing bottom half tasklets\n");
}
...
static int __init chr_dev_init(void)
{
	...
	if(request_irq(BUTTON_IRQ, irq_handler, IRQF_SHARED, "my_char_intr", (void *)(irq_handler)))
	{
		pr_info("error registering interrupt\n");
		goto intr_err;
	}
	else{
