 ...
 * Last modified : Monday 26 October 2020 10:04:46 AM IST
 * Description   : 
 * ***********************************************************************/
...
static unsigned int mouse_count = 0;
...
/* interrupt handler */
irqreturn_t irq_handler_mouse(int irq, void *dev_id)
{
	mouse_count++;
	pr_info("Mouse click captured: %d\n", mouse_count);
	return IRQ_HANDLED;
}
...
static int __init chr_dev_init(void)
{
	...
	if (request_irq(MOUSE_INTR, irq_handler_mouse, IRQF_SHARED, "my_intr", (void *)(irq_handler_mouse))){
		pr_info("error registering interrupt handler\n");
		goto irq_err;
	}
	pr_info("interrupt registered successfuly\n");
...
irq_err:
	free_irq(MOUSE_INTR, (void *)(irq_handler_mouse));
